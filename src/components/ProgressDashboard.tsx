...
const ProgressDashboard: React.FC<ProgressDashboardProps> = ({
  ...
  const generateChartBars = () => {
    ...
    const colors = ['#3b82f6', '#6366f1', '#8b5cf6', '#a855f7', '#d946ef', '#ec4899', '#10b981'];

    return sessionScores.map((score, idx) => {
  ...
  const getChartViewBox = () => {
    ...
    const width = Math.max(280, 40 + sessionScores.length * 34 + 20);
    return `0 0 ${width} 150`;
  };

  const chartBars = generateChartBars();
  const hasChartData = sessionScores && sessionScores.length > 0;
        ...
        >
          <div className="chart-header">
            ...
            <p className="chart-subtitle">
              {hasChartData ? `Last ${sessionScores?.length} sessions` : 'No data yet'}
            </p>
          </div>
